...
Item {
    ...
    property int size: Math.min(root.height, root.width) / 2
    property int arcBeginF: 0
    property int arcEndF: 270
    property int arcOffset: -135
    property int lineWidth: 20
    property string colorCircle: "#CC3333"

    property int hRR: HR.heartRateRestProp
    property int hRW: HR.actualHeartRateProp
    property int hRAe: HR.heartRateAerobicProp
    property int hRAeA: HR.heartRateAdvencedAerobicProp
    property int hRLa: HR.heartRateLacticProp
    property int hRAn: HR.heartRateAnaerobicProp
    property int hRAnA: HR.heartRateAdvencedAnaerobicProp
    ...
    onGivenValueChanged: {
        ...
        if (givenValue <= hRR)
            textWarn.text = qsTr("Too Low")
        else if (givenValue >= hRMax)
            textWarn.text = qsTr("Too High")
        else
            textWarn.text = ""
    }
    ...
    onWidthChanged: {
        ...
        var coef = 29 * Math.min(root.width / 120, root.height / 120)
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var correctGivenValue = (parent.givenValue
                                     <= parent.hRR) ? parent.hRR : parent.givenValue
            ...
            var testLowhRAe = parent.hRAe <= correctGivenValue
                    && correctGivenValue < parent.hRAeA
            var testLowhRAeA = parent.hRAeA <= correctGivenValue
                    && correctGivenValue < parent.hRLa
            var testLowhRLa = parent.hRLa <= correctGivenValue
                    && correctGivenValue < parent.hRAn
            var testLowhRAn = parent.hRAn <= correctGivenValue
                    && correctGivenValue < parent.hRAnA
            var testLowhRAnA = parent.hRAnA <= correctGivenValue
                    && correctGivenValue <= parent.hRMax
            ...
            ctx.lineCap = 'butt'
            ctx.beginPath()
            ctx.arc(x, y, lowRadius + lineWidth2 / 2 + 5 + lineWidth, 0,
                    positionArc)
            ...
            ctx.arc(x, y, lowRadius - ctx.lineWidth / 2,
                    (parent.hRR - parent.hRR) * coefArc,
                    (parent.hRAe - parent.hRR) * coefArc)
            ctx.strokeStyle = colorArray[0]
            ...
            ctx.arc(x, y, lowRadius - ctx.lineWidth / 2,
                    (parent.hRAe - parent.hRR) * coefArc,
                    (parent.hRAeA - parent.hRR) * coefArc)
            ctx.strokeStyle = colorArray[1]
            ...
            ctx.arc(x, y, lowRadius - ctx.lineWidth / 2,
                    (parent.hRAeA - parent.hRR) * coefArc,
                    (parent.hRLa - parent.hRR) * coefArc)
            ctx.strokeStyle = colorArray[2]
            ...
            ctx.arc(x, y, lowRadius - ctx.lineWidth / 2,
                    (parent.hRLa - parent.hRR) * coefArc,
                    (parent.hRAn - parent.hRR) * coefArc)
            ctx.strokeStyle = colorArray[3]
            ...
            ctx.arc(x, y, lowRadius - ctx.lineWidth / 2,
                    (parent.hRAn - parent.hRR) * coefArc,
                    (parent.hRAnA - parent.hRR) * coefArc)
            ctx.strokeStyle = colorArray[4]
            ...
            ctx.arc(x, y, lowRadius - ctx.lineWidth / 2,
                    (parent.hRAnA - parent.hRR) * coefArc,
                    (parent.hRMax - parent.hRR) * coefArc)
            ctx.strokeStyle = colorArray[5]
    ...
    Text {
        id: textPrincipal
        width: 300 * root.width / 400
        height: 100 * root.width / 400
        color: "#ffffff"
        text: "120"
        ...
        font.pixelSize: 29 * root.width / 400
        minimumPointSize: 10
    }
    Text {
        id: textbmp
        width: 75 * root.width / 400
        height: 22 * root.width / 400
        color: "#505050"
        text: "bmp"
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.top: textPrincipal.bottom
        font.pixelSize: 10 * root.width / 400
    ...
    Text {
        id: textWork
        width: 100 * root.width / 400
        height: 67 * root.width / 400
        color: "#ffffff"
        text: ""
        anchors.horizontalCenter: parent.horizontalCenter
